/* =========================================================================
 * RTP / volatility simulator — math verification for Critter Clash 1000.
 *
 * Runs the exact same engine code the browser uses and reports, per bet
 * mode: RTP, hit rate, battle/bonus frequency, max-win hits and win
 * distribution.
 *
...
function simulate(mode, n, seedOffset) {
  ...
  var battleSpins = 0, battleMultSum = 0, battleEvents = 0;
  ...
  for (var i = 0; i < n; i++) {
    ...
    var sawBonus = false, sawBattle = false;
    for (var e = 0; e < res.events.length; e++) {
      var ev = res.events[e];
      if (ev.type === 'fsTrigger') sawBonus = true;
      if (ev.type === 'battles') { sawBattle = true; battleEvents++; battleMultSum += ev.totalMult; }
    }
    if (sawBonus) bonuses++;
    if (sawBattle) battleSpins++;
  ...
  console.log('Battle rate:  %s%% of rounds | avg battle mult %sx',
    (100 * battleSpins / n).toFixed(1),
    battleEvents ? (battleMultSum / battleEvents).toFixed(2) : 'n/a');
...
console.log('CRITTER CLASH 1000 math verification — target RTP %s%%\n', (CFG.rtp * 100).toFixed(1));
simulate('base', rounds, 0);
simulate('buy', Math.floor(rounds / 10), 2);
simulate('superbuy', Math.floor(rounds / 10), 3);
